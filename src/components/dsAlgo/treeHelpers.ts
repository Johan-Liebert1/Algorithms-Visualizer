...
export const drawBinaryTreeNode = (
    ...
    if (parentNode instanceof TreeNode) {
        ...
        // of the parent node
        const { x: a, y: b } =
            binaryTreeNodesList[parentNode.uuid][side].children[1].position;
        x = a;
        y = b;
        ...
        // it's a heap node
        const { x: a, y: b } =
            heapNodesList[parentNode][side].children[1].position;
        x = a;
        y = b;
    ...
    if (isForTree) {
        ...
        const childNodeNum =
            side === "leftArrow" ? 2 * pNodeIdx : 2 * pNodeIdx + 1;
    ...
    rightArrow.rotate(
        -TREE_ARROW_ANGLE / (depth / 1.5),
        new paper.Point(rx, ry)
    );
...
export const drawTreeRoot = (
  ...
  */
    rootArrow.rotate(
        90,
        new paper.Point(rootArrow.position.x, rootArrow.position.y)
    );

    // now the arrow is at the bottom of the node
    rootArrow.position.y = drawnNode.rect.handleBounds.center.y;
    rootArrow.position.x =
        drawnNode.rect.handleBounds.center.x +
        30 +
        rootArrow.handleBounds.width / 2;
...
const treeInversionOneChild = async (
    ...
    const arrowName: arrowName =
        nodeToTranslate === "right" ? "leftArrow" : "rightArrow";
    const oppArrowName: arrowName =
        arrowName === "rightArrow" ? "leftArrow" : "rightArrow";

    const xAdder =
        nodeToTranslate === "left" ? NODE_SIZE / 2 : -(NODE_SIZE / 2);
...
export const animateBinaryTreeInversion = async (
    ...
    const isParentRoot = <boolean>(
        binaryTreeNodesList[parentNodeId].treeNode?.isRoot()
    );
    ...
    const { x: leftChildX, y: leftChildY } =
        binaryTreeNodesList[id1].node.rect.position;
    const { x: rightChildX, y: rightChildY } =
        binaryTreeNodesList[id2].node.rect.position;
