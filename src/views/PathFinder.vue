    ...
    <div class="outer-wrapper">
        ...
        <div class="wrapper" ref="gridContainer">
            <div class="algorithm-info has-text-centered">
                ...
                <Tooltip
                    :tooltipMessage="'Speed of the animation in milliseconds'"
                >
            ...
            >
                ...
                <label class="radio">
                    <input
                        type="radio"
                        name="maze-bias"
                        @change="changeMazeBias('horizontal')"
                    />
                    Horizontal Bias
                </label>
                <label class="radio">
                    <input
                        type="radio"
                        name="maze-bias"
                        @change="changeMazeBias('vertical')"
                    />
                    Vertical Bias
                </label>
            </div>
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            selectedMazeGenerationAlgorithm:
                mazeGenerationAlgorithms.RANDOMIZED_DFS,
            navbarButtons: [
                {
                    text: "Start",
    ...
    methods: {
        ...
        findShortestPath() {
            switch (this.selectedPathFindingAlgorithm) {
                ...
                default:
                    console.log(
                        this.selectedPathFindingAlgorithm,
                        "not yet implemented"
                    );
                    break;
            }
        ...
        highlightGrid(openCells: CellClass[], closedCells: CellClass[]) {
            for (const cell of openCells) {
                ...
                this.matrix[cell.row][cell.col].borderColor =
                    secondaryCellBorderColor;
            }
        ...
        colorFinalPath() {
            ...
            const interval = setInterval(() => {
                if (currentNode.previous !== null) {
                    this.matrix[currentNode.row][currentNode.col].color =
                        finalPathColor;
                    this.matrix[currentNode.row][currentNode.col].drawBorder =
                        false;
        ...
        initGrid(reset = true) {
            ...
            if (reset) {
                ...
                this.startNode =
                    this.matrix[prevStartNode.row][prevStartNode.col];
                this.endNode = this.matrix[prevEndNode.row][prevEndNode.col];
            }
        },

        cellClick(row: number, col: number) {
            if (this.startNode.row === row && this.startNode.col === col)
                return;
        ...
        initGridWrtCellSize() {
            const randCols = Math.floor(
                (window.innerWidth - 20) / this.CELL_SIZE
            );
