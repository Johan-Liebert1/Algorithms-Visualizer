              ...
              Right Click on Algorithm name to get more info
                ...
                >
                    <div
                        ...
                        v-for="(algo, index) in navbarButtons[
                            selectedMainDsAlgo.name
                        ]"
                        :key="index"
                        @click="algo.handler"
                        @contextmenu="
                            (e) => getMoreInfoAboutAlgorithm(e, algo.name)
                        "
                    >
                        {{
                ...
                <div class="legend">
                    <h1 class="is-size-4" style="align-self: center">Legend</h1>
...
export default defineComponent({
    ...
    setup() {
        const allMainDsAlgos = Object.values(allDsAlgosObject).map(
            (v) => v.name
        );
    ...
    data() {
        return {
            selectedMainDsAlgo:
                allDsAlgosObject.LINKED_LIST as selectedDsAlgoObjectType,
            ...
            navbarButtons: {
                [allDsAlgosObject.LINKED_LIST.name]: [
                    {
                        name: allDsAlgosObject.LINKED_LIST.algos
                            .REVERSING_LINKED_LIST.name,
                        handler: this.reverseLinkedList,
                    },
                ],
                [allDsAlgosObject.BINARY_TREES.name]: [
                    {
                        name: allDsAlgosObject.BINARY_TREES.algos
                            .INORDER_TRAVERSAL.name,
                        handler: () => this.traverseBinaryTree("inorder"),
                    },
                    {
                        name: allDsAlgosObject.BINARY_TREES.algos
                            .PREORDER_TRAVERSAL.name,
                        handler: () => this.traverseBinaryTree("preorder"),
                    },
                    {
                        name: allDsAlgosObject.BINARY_TREES.algos
                            .POSTORDER_TRAVERSAL.name,
                        handler: () => this.traverseBinaryTree("postorder"),
                    },
                    {
                        name: allDsAlgosObject.BINARY_TREES.algos
                            .INVERSION_OF_BINARY_TREE.name,
    ...
    methods: {
        ...
        clearCanvas() {
            ...
            this.linkedListStartPointer = {} as {
                pointer: paper.Group;
                index: number;
            };
        ...
        drawPointerOnNode(
            ...
            const { linkedListStartPointer, linkedListNodes, nullNode } =
                drawPointerOnNode(
                    this.linkedListNodes,
                    this.nullNode,
                    index,
            ...
            if (linkedListStartPointer)
                this.linkedListStartPointer = linkedListStartPointer;
        ...
        async translatePointer(
            ...
            const { linkedListNodes, nullNode, linkedListStartPointer } =
        ...
        async animateLinkedListNodeDeletion(
            indexToDelete: number
        ): Promise<void> {
            const { linkedListNodes, linkedListStartPointer, nullNode } =
        ...
        async addNodeToBinaryTree() {
            if (this.addNewNodeValue.toString().includes(",")) {
                let nodesToBeInserted = this.addNewNodeValue
                    .toString()
                    .split(",");

                if (!this.myBinaryTree.root) {
                    this.clearCanvas();
                    await this.myBinaryTree.insert(
                        Number(nodesToBeInserted[0])
                    );
                    this.drawBinaryTreeRoot(this.myBinaryTree.root);
            ...
            if (
                !this.myBinaryTree.root?.leftChild &&
                !this.myBinaryTree.root?.rightChild
            ) {
                this.clearCanvas();
                this.drawBinaryTreeRoot(this.myBinaryTree.root);
            }
        ...
        async deleteNodeFromBinaryTree(
            ...
            if (parent.treeNode?.leftChild === child.treeNode)
                side = "leftArrow";
            else side = "rightArrow";

            await animateTreeNodeDeletion(
                child,
                parent,
                side,
                this.animationSpeed
        ...
        checkNodeHit() {
            for (const object of Object.values(this.binaryTreeNodesList)) {
                for (const object2 of Object.values(this.binaryTreeNodesList)) {
                    if (object.node.rect.intersects(object2.node.rect))
        ...
        async addNodeToHeap() {
            ...
            if (this.addNewNodeValue.toString().includes(",")) {
                ...
                for (let idx = 0; idx < nodesToBeAdded.length; idx++) {
                    ...
                    const depth =
                        (
                            this.heapNodesList[parentNodeIndex]
                ...
                const depth =
                    (this.heapNodesList[parentNodeIndex].treeNode as TreeNode)
                        .depth + 1;
        ...
        async deleteFromHeap() {
            ...
            const side: arrowName =
                this.heapNodesList[parentIdx * 2] === child
                    ? "leftArrow"
                    : "rightArrow";
        ...
        async runOnMount(
            newSelectionValue: selectedDsAlgoObjectType | undefined
        ) {
    ...
    computed: {
        isLinkedListSelected(): boolean {
            return (
                this.selectedMainDsAlgo.name ===
                this.allDsAlgosObject.LINKED_LIST.name
            );
        },
        isTreeSelected(): boolean {
            return (
                this.selectedMainDsAlgo.name ===
                this.allDsAlgosObject.BINARY_TREES.name
            );
        },
        isHeapSelected(): boolean {
            return (
                this.selectedMainDsAlgo.name === this.allDsAlgosObject.HEAP.name
            );
        },
    },
