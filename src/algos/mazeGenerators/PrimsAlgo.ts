...
const primsMazeGenerator = async (
    ...
    const numberOfCells = Math.floor(
        (matrix.length / 2) * (matrix[0].length / 2)
    );
    ...
    while (Object.keys(partOfTheMaze).length < numberOfCells) {
        ...
        if (frontierCells.length === 0) {
            const randKey =
                Object.keys(partOfTheMaze)[
                    Math.floor(
                        Math.random() * Object.keys(partOfTheMaze).length
                    )
                ];
        ...
        if (neighbor !== currentCell) {
            const wallRemoved: CellClass = currentCell.removeWalls(
                neighbor,
                matrix
            );
